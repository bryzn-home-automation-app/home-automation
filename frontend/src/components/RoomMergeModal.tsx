...
import { mergeRoombaRooms, fetchRoombaCommands } from '../api/roomba';
...
/**
 * Confirmation dialog for merging rooms. EXPERIMENTAL, not-cleanly-reversible map
 * edit (never validated on hardware) — requires an explicit acknowledgement. After
 * enqueueing it watches the command status and refreshes the floor plan when done.
 */
export default function RoomMergeModal({ rooms, onClose }: Props) {
  ...
  const mutation = useMutation({
    mutationFn: () => mergeRoombaRooms(rooms.map((r) => r.id)),
  ...
  const names = rooms.map((r) => r.name ?? 'Unnamed room');
    ...
    >
      ...
      >
        ...
        <h3 className="mt-2 text-lg font-semibold text-apptext">
          Merge {rooms.length} rooms
        </h3>
        <p className="mt-2 text-sm leading-6 text-apptext-soft">
          Combine {names.join(', ')} into a single room.
        </p>

        <div className="mt-4 rounded-xl border border-amber-300/25 bg-amber-300/10 px-3 py-2.5 text-xs leading-5 text-amber-200">
          <span className="font-semibold">Experimental.</span> Merging hasn’t been verified on
          this robot and <span className="font-semibold">can’t be cleanly undone</span> from here —
          the combined room is newly recomputed, not a restore. Re-divide or reset the map from the
          iRobot app if needed.
        </div>
        ...
        {status && (
          ...
          >
            {status === 'OK'
              ...
              : status === 'FAILED'
                ? `The robot rejected the merge${queued?.detail ? `: ${queued.detail}` : '.'}`
                : 'Sending to the robot…'}
          </div>
        )}
        {mutation.isError && (
          <p className="mt-3 text-xs text-rose-300">Couldn’t queue the merge. Try again.</p>
        )}
        ...
        <div className="mt-6 flex justify-end gap-2">
          ...
          {status !== 'OK' && (
            ...
            >
              {submitting ? 'Merging…' : 'Merge rooms'}
            </button>
          )}
        </div>
      </div>
