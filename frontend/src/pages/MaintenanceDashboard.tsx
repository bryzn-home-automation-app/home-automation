...
function SummaryCards({ a }: { a: MaintenanceAnalytics | undefined }) {
  ...
  const cards = [
    ...
    { label: 'Upcoming', value: a?.scheduledCount ?? '...', icon: '📅', tone: tones[1], bgAlpha: 0.3 },
    { label: 'Completed', value: a?.completedCount ?? '...', icon: '✅', tone: tones[2], bgAlpha: 0.3 },
    { label: 'Lifetime Cost', value: formatCurrency(a?.totalLifetimeCost ?? 0), icon: '💰', tone: tones[3], bgAlpha: 0.24 },
    { label: 'This Year', value: formatCurrency(a?.thisYearCost ?? 0), icon: '📆', tone: tones[4], bgAlpha: 0.24 },
    { label: 'Monthly Avg', value: formatCurrency(a?.averageMonthlyCost ?? 0), icon: '📊', tone: tones[5], bgAlpha: 0.24 },
  ];
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/70 p-4 pt-[3vh] overflow-y-auto" onClick={onClose}>
      <div className="my-4 w-full max-w-xl rounded-[32px] border border-appborder bg-appsurface p-6 shadow-[0_20px_60px_var(--appshadow-lg)] sm:p-8" onClick={(e) => e.stopPropagation()}>
        <div className="mb-6 flex items-center justify-between">
          <h3 className="text-xl font-semibold text-apptext">{editing ? 'Edit Record' : 'Add Maintenance'}</h3>
